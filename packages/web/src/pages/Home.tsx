    ...
    <MarketingLayout>
      ...
      <section className="relative py-32 px-6 text-center overflow-hidden">
        ...
        <div className="relative max-w-3xl mx-auto space-y-8">
          <h1 className="text-5xl md:text-6xl font-bold text-text leading-[1.1] tracking-tight animate-fade-in">
            One assistant.
            <br />
            <span className="text-gradient">Every kind of work.</span>
          </h1>
          <p className="text-lg md:text-xl text-muted max-w-2xl mx-auto leading-relaxed animate-slide-up">
            Axel handles your calendar, your comms, and everything between —
            built to feel like a personal assistant, not a developer tool.
          </p>
          <p className="text-sm text-muted/70 animate-slide-up [animation-delay:100ms]">
            {RELEASE_EXPECTATION_COPY}
          </p>
          <div className="flex items-center justify-center gap-4 pt-2 flex-wrap animate-slide-up [animation-delay:200ms]">
            <Link to={waitlistSignupHref()}>
              <Button className="bg-accent-strong hover:bg-accent-strong/90 text-white px-8 py-3 text-base shadow-lg shadow-accent-strong/25 transition-all duration-200 hover:shadow-xl hover:shadow-accent-strong/30">
                Join waitlist
              </Button>
            </Link>
            <a href="#how-it-works">
              <Button
                type="button"
                variant="outline"
                className="border-border text-text hover:bg-surface-elevated hover:border-accent/30 px-8 py-3 text-base transition-all duration-200"
              >
                See how it works
              </Button>
            </a>
      ...
      <section
        id="how-it-works"
        className="relative py-20 px-6 bg-surface-raised bg-grain scroll-mt-20"
      >
        <div className="max-w-5xl mx-auto">
          <h2 className="text-2xl font-bold text-text text-center mb-4 tracking-tight">
            What Axel does for you
          </h2>
          <p className="text-muted text-center mb-12 max-w-xl mx-auto">
            Three pillars that keep your day running smoothly.
          </p>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {VALUE_CELLS.map(({ icon: Icon, headline, body }, i) => (
              <div
                key={headline}
                className="card-glow rounded-xl p-6 text-center md:text-left space-y-4"
                style={{ animationDelay: `${i * 100}ms` }}
              >
                <div className="inline-flex items-center justify-center w-12 h-12 rounded-xl bg-accent-strong/10 text-accent">
                  <Icon className="w-6 h-6" stroke={1.5} aria-hidden />
                </div>
        ...
        <div className="max-w-4xl mx-auto text-center space-y-10">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 text-left">
            {PRICING_TEASER.map(({ name, blurb }) => (
              <div
                key={name}
                className="card-glow rounded-xl p-6 group cursor-default"
              >
                <p className="font-semibold text-text mb-2 group-hover:text-accent transition-colors duration-200">
                  {name}
                </p>
      ...
      <section
        ...
        className="relative py-24 px-6 text-center scroll-mt-20"
      >
